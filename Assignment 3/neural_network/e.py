...
df.to_csv(os.path.join(output_folder_path, 'prediction_e.csv'), index=False)

weights = model.coefs_[:-1]
biases = model.intercepts_[:-1]
with open(os.path.join(output_folder_path,'mlp_weights.pkl'), 'wb') as f:
