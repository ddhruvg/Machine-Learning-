...
test_predictions = []
for h in hidden_layer_length :
    model = NeuralNetwork(M = 32 , n = 3072 , HiddenLayer = [h] , target_class = 36 , lr = 0.01 )
    model.fit(X_train , y_train , epochs = 200 )
    y_test_predictions = model.predict(X_test)
    test_predictions.extend(y_test_predictions)

df = pd.DataFrame(test_predictions, columns=['result'])
df.to_csv(os.path.join(output_folder_path, 'prediction_b.csv'), index=False)
